...
class BaseType {
  ...
  _getConvertFunction(field, fieldProperties, options) {
    ...
    if (options === undefined) {
      ...
      fieldProperties.hasOwnProperty('name') &&
    ...
    for (let key in fields) {
      if (fields.hasOwnProperty(key)) {
        ...
        if (this._fieldProperties[key].type == 'number') {
          let convertFunction = this._getConvertFunction(
            key,
            this._fieldProperties[key],
            options
          );
  ...
  format(format, field, unit) {
    let newFormat = this._getFormatFunction(
      field,
      this._fieldProperties[field],
      unit
    );
