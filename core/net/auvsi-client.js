    ...
    postTelemetry(telemetry, callback) {
        ...
        for (let i = 0; i < standardKeys.length; i++) {
            if (!telemetry.hasOwnProperty(standardKeys[i])) {
                callback(new Error('Telemetry is missing key \'' +
                        standardKeys[i] + '\''));
                return;
            } else if (typeof telemetry[standardKeys[i]] !==
                    STANDARD_TELEMETRY[standardKeys[i]]) {
                callback(new Error('key \'' + standardKeys[i] +
                        '\' is the wrong type'));
                return;
            }
        }
    ...
    postTarget(target, callback) {
        ...
        for (let i = 0; i < keys.length; i++) {
            if (!STANDARD_TARGETS.hasOwnProperty(keys[i])) {
                callback(new Error('Target contains extra key \'' + keys[i] +
                        '\''), null);
                return;
            } else if (typeof target[keys[i]] !== STANDARD_TARGETS[keys[i]]) {
                callback(new Error('key \'' + keys[i] +
                        '\' is the wrong type'));
                return;
            }
        }
